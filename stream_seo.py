...
# ============ Tab 3: 數據分析 ============
    ...
    if not history_records:
        ...
        if rankings:
            ...
            if tracked_my_sites and tracked_competitors:
                st.markdown("### 🥊 競爭分析：關鍵字爭奪")

                col1, col2 = st.columns(2)
                with col1:
                    my_site_select = st.selectbox("選擇我的網站", tracked_my_sites, key="compete_my_site")
                with col2:
                    competitor_select = st.selectbox("選擇競爭對手", tracked_competitors, key="compete_competitor")

                if my_site_select and competitor_select:
                    # 分析每個關鍵字的競爭情況
                    winning = []  # 我贏
                    losing = []  # 我輸
                    both_ranked = []  # 雙方都有排名
                    only_me = []  # 只有我有排名
                    only_competitor = []  # 只有對手有排名
                    neither = []  # 都沒排名

                    for item in rankings:
                        keyword = item.get("keyword")
                        my_rank = item.get(my_site_select)
                        comp_rank = item.get(competitor_select)

                        if my_rank is None and comp_rank is None:
                            neither.append(keyword)
                        elif my_rank is None:
                            only_competitor.append({"keyword": keyword, "competitor_rank": comp_rank})
                        elif comp_rank is None:
                            only_me.append({"keyword": keyword, "my_rank": my_rank})
                        else:
                            both_ranked.append({
                                "keyword": keyword,
                                "my_rank": my_rank,
                                "comp_rank": comp_rank,
                                "diff": comp_rank - my_rank
                            })
                            if my_rank < comp_rank:
                                winning.append({"keyword": keyword, "my_rank": my_rank, "comp_rank": comp_rank})
                            elif my_rank > comp_rank:
                                losing.append({"keyword": keyword, "my_rank": my_rank, "comp_rank": comp_rank})

                    # 顯示統計
                    stat_cols = st.columns(5)
                    with stat_cols[0]:
                        st.metric("🏆 我贏", len(winning))
                    with stat_cols[1]:
                        st.metric("😢 我輸", len(losing))
                    with stat_cols[2]:
                        st.metric("✅ 只有我", len(only_me))
                    with stat_cols[3]:
                        st.metric("⚠️ 只有對手", len(only_competitor))
                    with stat_cols[4]:
                    ...
                    compete_tabs = st.tabs([
                        ...
                        f"😢 我輸 ({len(losing)})",
                        f"✅ 只有我 ({len(only_me)})",
                        f"⚠️ 只有對手 ({len(only_competitor)})",
                        f"❌ 都沒排名 ({len(neither)})"
                    ])

                    with compete_tabs[0]:
                        if winning:
                            st.success(f"🎉 以下 {len(winning)} 個關鍵字我排名領先！")
                            win_df = pd.DataFrame(winning)
                            win_df.columns = ["關鍵字", "我的排名", "對手排名"]
                            win_df["優勢"] = win_df["對手排名"] - win_df["我的排名"]
                    ...
                    with compete_tabs[1]:
                        if losing:
                            st.error(f"⚠️ 以下 {len(losing)} 個關鍵字需要加強！")
                            lose_df = pd.DataFrame(losing)
                            lose_df.columns = ["關鍵字", "我的排名", "對手排名"]
                            lose_df["落後"] = lose_df["我的排名"] - lose_df["對手排名"]
                    ...
                    with compete_tabs[2]:
                        if only_me:
                            st.success(f"✅ 以下 {len(only_me)} 個關鍵字只有我上榜！")
                            only_me_df = pd.DataFrame(only_me)
                            only_me_df.columns = ["關鍵字", "我的排名"]
                            st.dataframe(only_me_df.sort_values("我的排名"), use_container_width=True, hide_index=True)
                        else:
                            st.info("沒有獨佔的關鍵字")

                    with compete_tabs[3]:
                        if only_competitor:
                            st.warning(f"⚠️ 以下 {len(only_competitor)} 個關鍵字只有對手上榜！")
                            only_comp_df = pd.DataFrame(only_competitor)
                            only_comp_df.columns = ["關鍵字", "對手排名"]
                            st.dataframe(only_comp_df.sort_values("對手排名"), use_container_width=True,
                                         hide_index=True)
                        else:
            ...
            # 建立每個網站的詳細分析
            site_keyword_details = {}

            for site in all_sites:
                site_keyword_details[site] = {
                    "top3": [],
                    "top10": [],
                    "top20": [],
                    "top30": [],
                ...
                for item in rankings:
                    ...
                    if rank is None:
                        site_keyword_details[site]["na"].append(keyword)
                    else:
                        if rank <= 3:
                            site_keyword_details[site]["top3"].append({"keyword": keyword, "rank": rank})
                        elif rank <= 10:
                            site_keyword_details[site]["top10"].append({"keyword": keyword, "rank": rank})
                        elif rank <= 20:
                            site_keyword_details[site]["top20"].append({"keyword": keyword, "rank": rank})
                        elif rank <= 30:
                            site_keyword_details[site]["top30"].append({"keyword": keyword, "rank": rank})

                        if rank > analysis_warning_threshold:
                            site_keyword_details[site]["warning"].append({"keyword": keyword, "rank": rank})
            ...
            if selected_site_detail:
                details = site_keyword_details[selected_site_detail]
            ...
            for site in all_sites:
                site_stats = {
                    ...
                    "第2頁(11-20)": 0,
                    "21-30名": 0,
                    f">{analysis_warning_threshold}名": 0,
                    "未上榜": 0,
                    "平均排名": 0
                }
                ...
                for item in rankings:
                    ...
                    if rank is None:
                        ...
                        if rank <= 3:
                            ...
                            site_stats["首頁(4-10)"] += 1
                        elif rank <= 20:
                            site_stats["第2頁(11-20)"] += 1
                        elif rank <= 30:
                            site_stats["21-30名"] += 1

                        if rank > analysis_warning_threshold:
                            site_stats[f">{analysis_warning_threshold}名"] += 1
